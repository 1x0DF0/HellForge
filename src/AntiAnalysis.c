...
// thread ID of the running MouseClicksLogger thread (0 when idle)
volatile DWORD g_dwMouseThreadId = 0;
...
BOOL MouseClicksLogger() {
    ...
    g_dwMouseThreadId = GetCurrentThreadId();
    ...
    // process unhandled events; exits when WM_QUIT is posted by AntiAnalysis
...
BOOL DeleteSelf() {
	...
	pRename->FileNameLength = sizeof(NEW_STREAM) - sizeof(WCHAR);
	RtlCopyMemory(pRename->FileName, NewStream, sizeof(NEW_STREAM) - sizeof(WCHAR));
...
BOOL AntiAnalysis(DWORD dwMilliSeconds) {
	...
	while (i <= 10) {
		...
		// Unblock the GetMessageW loop in MouseClicksLogger so the thread exits cleanly.
		// Without this the thread stays alive indefinitely after the handle is closed.
		if (g_dwMouseThreadId)
			PostThreadMessageW(g_dwMouseThreadId, WM_QUIT, 0, 0);
